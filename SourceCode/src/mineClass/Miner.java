...
public class Miner implements OptionHandler{
    ...
    int GlobalTFP = 0;               //false zero-day attack per chunk 
    int GlobalTFN = 0;               //False not zero-day attack per chunk 
    int GlobalZeroAttack = 0;               //Number zero-day attack per chunk 
    
    int GlobalZTP = 0;               //FP per chunk (my method)
    int GlobalZTN = 0;               //FN per chunk (my method)
    int GlobalZFP = 0;               //FP per chunk (my method)
    int GlobalZFN = 0;               //FN per chunk (my method)
    Classifier cls;
    ...
    // Pos and Neg Classes    
//    List<Double> posClasses = Arrays.asList(3.0,4.0);   /*Ahmad*/ 
    //-F D:\hp_ids_43Attack_unpatchedServer\bidi_43attacks_unpatchedServer\datafile-qi8tgoxuk55.c0.d64.C23.N56.t100-shuffled-T200 -S 100 -T 50 -C 20 -N 15 -L 3 -M 15 -B reasc.ReascCtrl -K 15 -P 90
    //-F D:\hp_ids_43Attack_unpatchedServer\ngram_43attacks_unpatchedServer_19Features\datafile-qi8tgoxuk55.c0.d65.C43.N56.t100-shuffled-T200_RForest19.da3ch30eps6dclp -S 100 -T 50 -C 20 -N 15 -L 3 -M 15 -B reasc.ReascCtrl -K 15 -P 90
    //-F D:\hp_ids_43Attack_unpatchedServer\ngram_43attacks_unpatchedServer_allFeatures\datafile-qi8tgoxuk55.c0.d65.C43.N56.t100-shuffled-T200 -S 100 -T 50 -C 20 -N 15 -L 3 -M 15 -B reasc.ReascCtrl -K 15 -P 90
    //-F D:\hp_ids_43Attack_unpatchedServer\ngram_43attacks_unpatchedServer_allFeatures\datafile-qi8tgoxuk55.c0.d65.C43.N56.t100-shuffled-T200.da3ch30eps6dclp -S 100 -T 50 -C 20 -N 15 -L 3 -M 15 -B reasc.ReascCtrl -K 15 -P 90
//    -F D:\WebsiteFingerPrinting\newData_feb9_bidi_ngram_zeroday\DAE2\datafile-dp6uce47k28.c0.d64.C23.N29.t100.T100.D1.E1.F1.G1.H1.I1.B16.J8.K300.L0.05.M100.A1.V0.P0.G0.l0.0.b600.u200.Q16.w16.W16shuffled-T200
//    List<Double> posClasses = Arrays.asList(12.0, 13.0, 14.0, 15.0, 16.0, 17.0, 18.0, 19.0, 20.0, 21.0, 22.0, 23.0, 24.0, 25.0, 26.0, 27.0, 28.0, 29.0, 30.0, 31.0, 32.0, 33.0,
//    34.0, 35.0, 36.0, 37.0, 38.0, 39.0, 40.0, 41.0, 42.0, 43.0, 44.0, 45.0, 46.0, 47.0, 48.0, 49.0, 50.0,51.0, 52.0, 53.0, 54.0);/*Ahmad*/
    List<Double> posClasses = Arrays.asList(4.0,5.0,6.0,7.0,8.0,9.0,10.0,11.0, 13.0, 14.0, 15.0, 16.0, 17.0, 18.0, 19.0, 20.0);/*Ahmad*/
//    List<Double> posClasses = Arrays.asList(1.0);/*Ahmad*/
        ...
        for(int i = 0; i < outlierList.numInstances(); i ++)
        {
            ...
            if(outli.EPrediction.Isoutlier == false) //need to remove and predict accuracy immediately
            {
                if(TheNewClass[(int)outli.classValue()] == true){
                    if(posClasses.contains(outli.classValue())){ // Classified as not Zero-day attack while it is a Z /*Ahmad*/
                        outli.cfn = true;
                    }
                    else{                                  //Classified as not Zero-day attack and it's not Zero-day attack
                        outli.ctn = true;
                    }
                }
                else{
                ...
                if(outli.classValue() != outli.EPrediction.Predclass)
                {
                    ...
                    if(TheNewClass[(int)outli.classValue()] == true)
                    {
                        outli.fn = true;
                        outli.isNovel = true;
                        if(posClasses.contains(outli.classValue())){
                            outli.tfn = true;
                            outli.isZeroAttack= true;
                        }
                    }
                }
    ...
    {          
        ...
        */
        ...
        minst.predictedClass=votedClass;    /*Ahmad*/
//        minst.predictedClass=cls.classifyInstance(minst);   /*Ahmad*/
        ...
        //{
            if(!minst.EPrediction.Isoutlier) //classify immediately
            {
                if(TheNewClass[(int)minst.classValue()] == true){
                    if(posClasses.contains(minst.classValue())){ // Classified as not Zero-day attack while it is a Z /*Ahmad*/
                        minst.cfn = true;
                    }
                    else{                                  //Classified as not Zero-day attack and it's not Zero-day attack
                        minst.ctn = true;
                ...
                if(votedClass != minst.classValue())
                {
                    ...
                    if(TheNewClass[(int)minst.classValue()] == true) //false negative, novel class instance
                    {
                        minst.fn = true;
                        minst.isNovel = true;
                        if(posClasses.contains(minst.classValue())){
                            minst.tfn = true;
                            minst.isZeroAttack= true;
                        }
                    }
                }
                ...
                if(novelClusters != null)
                {
                    ...
                    if(inside) //then classify as novel class
                    {
                        ...
                        if(TheNewClass[(int)minst.classValue()] == true) 
                        {
                            ...
                            if(posClasses.contains(minst.predictedClass)){      //Classified as Zero-day attack  /*Ahmad*/                                                            
                                if (posClasses.contains(minst.classValue())) {
                                    minst.ctp = true;                                    
                                }
                                else{
                                ...
                                if (!posClasses.contains(minst.classValue())) {  //Classified as not Zero-day attack
                                    minst.ctn = true;                                    
                                }
                                else{
                                    minst.cfn = true;
                        ...
                        else //false positive
                        {
                            ...
                            else{                                  //Classified as not Zero-day attack
                                minst.ctn = true;
                            }
                            minst.fp = true;
                            minst.err = true;
        ...
        if(outlierList.minstance(id).Id + this.Tc <= minst.Id) //it must be classified now
        {
            ...
            if(TheNewClass[(int)outli.classValue()] == true){ /*Ahmad*/
                if(posClasses.contains(outli.classValue())){ // Classified as not Zero-day attack while it is a Z /*Ahmad*/
                    outli.cfn = true;
                }
                else{                                  //Classified as not Zero-day attack and it's not Zero-day attack
        ...
        if(totalUnComitted > this.Minpts && (minst.Id > this.Minpts + LastCheck)) // then check for novel classes
        {
            ...
            if(L > 1)
            {
                ...
                int[] nearestClassVote = new int[outlierList.numClasses()]; /*Ahmad*/
                ...
                for(int i = 0; i < es; i ++)
                {
                    ...
                    for(int j = 0; j < allClusters.length; j  ++)
                        ...
                        {
                            novelVote[j] ++;
                            nearestClassVote[allClusters[j].nearestClass]++;    /*Ahmad*/
                        }
...
//                    //(Approach 1) ArgMax: Get the most Frequent pseudopoint Classes among outliers /*Ahmad*/
//                    int mjClass = 0;
//                    int maxValue = nearestClassVote[mjClass];
//                    for (int i = 0; i < nearestClassVote.length; i++) {
//                      int elem = nearestClassVote[i];
//                      if (elem > maxValue) {
//                        maxValue = elem;
//                        mjClass = i;
//                      }
//                    }   /*Ahmad*/                    
                    ...
                    //(Approach 2) Positive; if at least one model voted for positive class /*Ahmad*/                    
                    int mjClass = -1;
                    for (int i = 0; i < nearestClassVote.length; i++) {
                        if(nearestClassVote[i]>0){
                        if(posClasses.contains((double)i)){
                            mjClass = i;
                            break;
                        }}
                    }   /*Ahmad*/
                    ...
                    for(int i = 0; i < outlierList.numInstances();  i ++)
                    {
                        ...
                        if(out == es)
                        {
                            p.predictedClass=mjClass; /*Ahmad*/
//                            p.predictedClass=cls.classifyInstance(p);   /*Ahmad*/
                            ...
                            if(isNewClass) //new class really appeared
                            {
                                ...
                                {
                                    p.isNovel = true;
                                    if (posClasses.contains(p.classValue())){ 
                                        p.isZeroAttack= true; 
                                    }
                                    if(posClasses.contains(p.predictedClass)){      //Classify as Zero-day attack /*Ahmad*/
                                        if (posClasses.contains(p.classValue())) 
                                            p.ctp = true;   
                                        else
                                            p.cfp = true;                                        
                                ...
                                { // our prediction is wrong (this instance does not belong to a new class)
                                    if(posClasses.contains(p.predictedClass)){  //Classify as Zero-day attack while it's not /*Ahmad*/
                                        p.cfp = true;
                                    }
                                    else{
                                        p.ctn = true;
                            ...
                            {
                                if(posClasses.contains(p.predictedClass)){  /*Ahmad*/
                                    p.cfp = true;
                                }
                                else{
                                    p.ctn = true;
    ...
    void printResult(int chunkSize, int K, int Minpts)
    {
        ...
        this.GlobalTFP = this.GlobalTFN = this.GlobalZeroAttack = 0;
        this.GlobalZTP = this.GlobalZTN = this.GlobalZFP = this.GlobalZFN = 0;
        int total = 0;
        ...
        for(chunkno = 0; chunkno < Results.AllResult.size(); chunkno ++)
        {
            ...
            this.GlobalZeroAttack+= st.zeroAttack;
            this.GlobalTFP += st.tfp;
            this.GlobalTFN += st.tfn;

            this.GlobalZTP += st.ctp;
            this.GlobalZTN += st.ctn;
            this.GlobalZFP += st.cfp;
            this.GlobalZFN += st.cfn;
        ...
        try{
            ...
            {                
                 out = new BufferedWriter(new FileWriter("ECHO_"+ "V" + Constants.VERSION +"-"+(InFile.lastIndexOf("/")==-1?InFile.substring(InFile.lastIndexOf("\\")+1):InFile.substring(InFile.lastIndexOf("/")+1))+"-T"+this.Tl+
                 "-C"+this.Tc+ "-S"+this.ChunkSize+"-U"+Constants.TAU+"-D"+Constants.DYNAMIC+"-W"+Constants.MAXSLIDINGWINDOWSIZE+"-A"+Constants.SENSITIVITY+"-G"+Constants.GAMMA+"-R"+Constants.RELAXATION+"-E"+Constants.AEparam.replaceAll("\\s", "-")+ ".tmpres"));
                 out.write("Chunk#\tFP\tFN\tNC\tErr\tGlobErr\n" );
            }
            //else
            {
                 out = new BufferedWriter(new FileWriter("ECHO_"+ "V" + Constants.VERSION +"-"+(InFile.lastIndexOf("/")==-1?InFile.substring(InFile.lastIndexOf("\\")+1):InFile.substring(InFile.lastIndexOf("/")+1))+"-T"+this.Tl+
                 "-C"+this.Tc+ "-S"+this.ChunkSize+"-U"+Constants.TAU+"-D"+Constants.DYNAMIC+"-W"+Constants.MAXSLIDINGWINDOWSIZE+"-A"+Constants.SENSITIVITY+"-G"+Constants.GAMMA+"-R"+Constants.RELAXATION+"-E"+Constants.AEparam.replaceAll("\\s", "-")+".tmpres",true));
                String output = chunkno+"\t"+
                        st.ctp+"\t"+
                        st.cfp+"\t"+
                        st.ctn+"\t"+
                        st.cfn+"\t"+
    ...
    {
        try{
            BufferedWriter out = new BufferedWriter(new FileWriter("ECHO_"+ "V" + Constants.VERSION +"-"+(InFile.lastIndexOf("/")==-1?InFile.substring(InFile.lastIndexOf("\\")+1):InFile.substring(InFile.lastIndexOf("/")+1))+"-T"+this.Tl+
                 "-C"+this.Tc+ "-S"+this.ChunkSize+"-U"+Constants.TAU+"-D"+Constants.DYNAMIC+"-W"+Constants.MAXSLIDINGWINDOWSIZE+"-A"+Constants.SENSITIVITY+"-G"+Constants.GAMMA+"-R"+Constants.RELAXATION+"-E"+Constants.AEparam.replaceAll("\\s", "-")+".res"));
            ...
            double tfn = 100 * (double)GlobalTFN / (double)(GlobalFN);            
            double zeroattacks = GlobalZeroAttack;
            
            double ctp = 100 * (double)GlobalZTP / (double)(GlobalZFN+GlobalZTP);
            double cfp = 100 * (double)GlobalZFP / (double)(GlobalZTN+GlobalZFP);
            double ctn = 100 * (double)GlobalZTN / (double)(GlobalZFP+GlobalZTN);
            double cfn = 100 * (double)GlobalZFN / (double)(GlobalZTP+GlobalZFN);
            ...
            out.write("\nZTP%\tFP%\tTN%\tFN%\ttotal\n");
            out.write(ctp+"\t"+cfp+"\t"+ctn+"\t"+cfn+"\t"+(GlobalZTP+GlobalZFP+GlobalZTN+GlobalZFN));
    ...
    void Classify(String[] argv) throws IOException
    {
        ...
        */
        ...
        try{
//            Instances auxdata = new Instances(new BufferedReader(new FileReader(infile+".aux.arff")));
//            if (auxdata.classIndex() == -1)
//                auxdata.setClassIndex(auxdata.numAttributes() - 1);
//            cls = new J48();
//            cls.buildClassifier(auxdata);

            in = new BufferedReader(new FileReader(infile+".arff"));
            rawData = new Instances(in,this.ChunkSize); /*Ahmad*/
            ...
            {
                while(rawData.readInstance(in))
                {
                    //System.out.println(data.numInstances() + " " + ((Instance)data.instance(data.numInstances() - 1)).value(0));
                    if(rawData.numInstances() == this.ChunkSize)
                    break;
                }
                
                //Build or update Autoencoder
                if (i==0){
                    data = ae.TrainAndTransform(rawData, false);
                }
                else{
                    data = ae.TrainAndTransform(rawData, true);
                }
        ...
        */
        ...
        Instances dataPointsWindow=new Minstances(rawData, 0);
        ...
        */
        ...
        int estimatedCP = -1;
        int estimatedCPinRawData = -1;
        try{
             while(rawData.readInstance(in))
             {                
                data = ae.Transform(rawData);               
                dataPointsWindow.add(rawData.instance(0));
                rawData.delete(0);
...
//                            estimatedCP = changeDetector.detectChangenonpar();
                            estimatedCP = changeDetector.detectChangeNonparMultidim(testData);
//                            estimatedCPinRawData = changeDetector.detectChangeNonparMultidim(dataPointsWindow);
                        }
                        else
                        {
                            estimatedCP = -1;
...
//                            estimatedCPinRawData = changeDetector.detectChangeNonparMultidim(dataPointsWindow);
                ...
                //time to train AE
//                if(dataPointsWindow.numInstances() - estimatedCPinRawData > 20 && estimatedCPinRawData != -1)
                if(dataPointsWindow.numInstances()>100)
                {  
//                        System.out.println("Updating AE.."); 
                        ae.UpdateAE(dataPointsWindow);
//                        ae.UpdateAE(new Instances(dataPointsWindow,estimatedCPinRawData,dataPointsWindow.numInstances()-estimatedCPinRawData));
                        dataPointsWindow.delete();
                }
                if(dataPointsWindow.numInstances()>3000){
                    dataPointsWindow.delete();
                }
//                if(/*trainData.numInstances() == this.ChunkSize*/estimatedCP != -1)
                if(trainData.numInstances() > estimatedCP && estimatedCP != -1)
                {
//                    if(dataPointsWindow.numInstances() - estimatedCP > 20)
////                    if(dataPointsWindow.numInstances()>100)
//                    {
////                        System.out.println("Updating AE..");
//                        ae.UpdateAE(new Instances(dataPointsWindow,estimatedCP,dataPointsWindow.numInstances()-estimatedCP));
////                        ae.UpdateAE(dataPointsWindow);
//                        for(int s=0;s<=estimatedCP; s++){
//                            dataPointsWindow.delete(0);
//                        }
//                    }
             ...
             System.out.println("\n\n% of labeled data = " + String.format("%.2f", ((double)numOfLabeledInst/(double)(numOfLabeledInst + numOfUnlabeledInst))*100)+"%");
             System.out.println("\nTotal time needed: " + (long)(System.currentTimeMillis()-CurTime) + " milliseconds");
             in.close();
             Allinst.close();
            }//try
            catch(Exception e)
  ...
  public Enumeration listOptions() {
    ...
    newVector.
	addElement(new Option("\tSet maximum allowable window size" +
			      "\t(default 3000)",
			      "W", 1, "-W <chunk size>"));
    newVector.
	addElement(new Option("\tSet the sensitivity parameter." +
			      "\t(default 0.001)",
			      "A", 1, "-A <sensitivity parameter>"));
    newVector.
	addElement(new Option("\tSet the value of gamma." +
			      "\t(default 0.5)",
			      "G", 1, "-G <value for gamma>"));
    newVector.
	addElement(new Option("\tSet the relaxation parameter." +
			      "\t(default is the same as sensitivity parameter used)",
			      "R", 1, "-R <relaxation parameter>"));
    newVector.
	addElement(new Option("\tSet the Autoencoder parameters: \"AEfile.py gauss/dropout learningrate n_Hdn_layers\" e.g., \"server.py gauss 0.1 2\"",
			      "E", 1, "-E <Autoencoder parameters>"));
    /*
    newVector.
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    String MaxWindowSize = Utils.getOption('W', options);
    if (MaxWindowSize.length() != 0) {
        Constants.MAXSLIDINGWINDOWSIZE = Integer.parseInt(MaxWindowSize);
      }
     else {
      Constants.MAXSLIDINGWINDOWSIZE = 3000;
    }
    
    String alpha = Utils.getOption('A', options);
    if (alpha.length() != 0) {
        Constants.SENSITIVITY = Double.parseDouble(alpha);
      }
     else {
      Constants.SENSITIVITY = 0.001;
    }
    
    String gamma = Utils.getOption('G', options);
    if (gamma.length() != 0) {
        Constants.GAMMA = Double.parseDouble(gamma);
      }
     else {
      Constants.GAMMA = 0.5;
    }
    
    String relaxation = Utils.getOption('R', options);
    if (relaxation.length() != 0) {
        Constants.RELAXATION = Double.parseDouble(relaxation);
      }
     else {
      Constants.RELAXATION = Constants.SENSITIVITY;
    }
    
    String AEparam = Utils.getOption('E', options);
    if (AEparam.length() != 0) {
        Constants.AEparam = AEparam;
      }
     else {
      Constants.AEparam = "";
    }
  ...
  public String [] getOptions() {
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        try{
         fa= new FileAppender(sl, "ECHO_"+ "V" + Constants.VERSION +"-"+(ev.InFile.lastIndexOf("/")==-1?ev.InFile.substring(ev.InFile.lastIndexOf("\\")+1):ev.InFile.substring(ev.InFile.lastIndexOf("/")+1))+"-T"+ev.Tl+
                 "-C"+ev.Tc+ "-S"+ev.ChunkSize+"-U"+Constants.TAU+"-D"+Constants.DYNAMIC+"-W"+Constants.MAXSLIDINGWINDOWSIZE+"-A"+Constants.SENSITIVITY+"-G"+Constants.GAMMA+"-R"+Constants.RELAXATION+"-E"+Constants.AEparam.replaceAll("\\s", "")+".log",false);
        }
        catch(IOException e)
        {
            System.out.println(e.getMessage());
